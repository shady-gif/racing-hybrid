...
function loadCarModel(ammo, scene, carComponents, wheelPositions, onModelLoaded) {
  ...
  loader.load(
    '/models/futuristic-green-supercar.glb',
    (gltf) => {
      const importedModel = gltf.scene;
      const carModel = new THREE.Group();
      carModel.name = 'FuturisticGreenSupercarPhysicsRoot';
      normalizeFuturisticSupercarModel(importedModel);
      carModel.add(importedModel);

      importedModel.traverse((node) => {
        if (node.isMesh) {
          node.castShadow = true;
          node.receiveShadow = true;
          if (isCarBodyMesh(node)) {
            node.material = createPastelYellowBodyMaterial();
          } else if (Array.isArray(node.material)) {
            node.material = node.material.map(prepareImportedMaterial);
          } else if (node.material) {
            node.material = prepareImportedMaterial(node.material);
          }
        }
      });

      // The imported Bugatti is a merged mesh, so physics wheels use invisible
      // transform holders while the model's baked wheels remain on the chassis.
      for (let i = 0; i < wheelPositions.length; i++) {
        const wheelTransform = new THREE.Object3D();
        scene.add(wheelTransform);
        carComponents.wheelMeshes[i] = wheelTransform;
      }

      scene.add(carModel);
      carComponents.carModel = carModel;
      console.log('Futuristic green supercar loaded successfully');

      if (onModelLoaded) onModelLoaded(carComponents);
    },
    undefined,
    (error) => {
      console.error('Error loading futuristic green supercar:', error);
    }
  );
}

function normalizeFuturisticSupercarModel(model) {
  const bakedShadow = model.getObjectByName('car_shadow_38');
  bakedShadow?.parent?.remove(bakedShadow);
  const legacyClearcoat = model.getObjectByName('clearcoat_34');
  legacyClearcoat?.parent?.remove(legacyClearcoat);
  ...
  model.scale.setScalar(longestHorizontal > 0 ? targetLength / longestHorizontal : 1);
  model.updateMatrixWorld(true);

  const scaledBounds = new THREE.Box3().setFromObject(model);
  const center = scaledBounds.getCenter(new THREE.Vector3());
  model.position.set(-center.x, -0.38 - scaledBounds.min.y, -center.z);
}

function prepareImportedMaterial(material) {
...
function isCarBodyMesh(node) {
  let current = node;
  while (current) {
    if (current.name === 'car_body_36') return true;
    current = current.parent;
  }
  return false;
}

function createPastelYellowBodyMaterial() {
